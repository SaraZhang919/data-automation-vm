...
DESCRIPTIONS={
 ...
 'Clarity Snapshots':('行为辅助','每周二17:00 JST仅取一条项目总体指标快照；覆盖此前72小时，不是完整一周。旧24小时快照保留原始窗口标记；不保存URL/设备拆分。'),
...
def update_guide(store,report=None):
    ...
    topics=[
        ...
        ('LLM发送内容','发送结构化汇总，不发送服务器原始日志、录屏、完整SF文件、密钥或整本表格。表头及共用日期去重；无法比较的重复行按原因汇总；Clarity只发送最新总体快照。普通分析保留所选GA/GSC页面和关键词级聚合指标；按需分析才读取指定历史。'),
        ('Clarity频率与窗口','每周二17:00 JST采集一次，无URL/设备维度，只存总体指标。API最多回看72小时，因此每周快照不等于完整周总数；日报引用最近快照并注明时间，不每日调用。'),
