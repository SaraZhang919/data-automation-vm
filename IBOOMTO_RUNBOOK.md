...
selection_reason preserves all matching categories with | separators. Baseline dates, counts, daily deltas, growth ratio, pool coverage and rule version are recorded. Current metrics remain exact API period metrics; do not sum selected queries to reproduce site totals. No manual keyword watchlist is implemented. LLM reports use the relevance policy below, never the raw 5,000-query pool; candidate selection itself uses no LLM. Historical AI text is not automatically regenerated for this migration.


## Relevance and usage audit (2026-09-10)

Source collection remains the 5,000-query pool and selected union of up to 300 queries per language/period. Daily LLM evidence now contains a summary of every selected query and representative detail: all rows when there are at most 20; otherwise up to five each by clicks, impressions, new-candidate impressions and growth delta, deduplicated. Explicit red/yellow or P1/P2 query alerts and observed growth of at least 100 impressions/day plus 200% remain included without a hard cap. Summary counts label overlapping categories and selected-only totals. Weekly/monthly retain the selected union with only analytical fields. Deep requests retain their requested history. Comparison input uses the latest current_end independently per source/language/period and retains both daily baseline types. This does not collapse GA and GSC onto a shared date. Storage may still read history for upserts and comparisons; those reads do not imply LLM transmission.

AI Usage is one row per paid-request attempt, persisted before sending. module_token_estimates and estimated_input_tokens use o200k_base locally and are not billed measurements. input_tokens/output_tokens/total_tokens come from the provider. phase distinguishes full, partition and synthesis. AI Run Usage sums all attempts for one logical report, including cached partition reuse; report-history cache hits record zero calls. An uncertain timeout is not retried automatically and its fee stays unknown. Estimates use the GPT-5.6 Sol public rates checked on September 10: ordinary input $4/M, cached input $0.40/M, cache writes $5/M, output $20/M, with long-context multipliers. Output includes reasoning, so it is not charged twice. Model mismatch or dates after November 21 require pricing review rather than silently extending the rate. These are cost estimates, not invoice reconciliation. No spending cap was introduced.

API Usage records application-level Google attempts by API/operation/property, status errors, explicit retries, observed 60-second peaks and GA returned propertyQuota. It counts this process, not other applications sharing the account/project. Google authentication and final telemetry writes are excluded from the Sheets snapshot. runtime/api-usage.json records exit-time counters even after a fatal run error, and Actions summary prints request and retry counts. No URLs, query text, credentials or raw response data are stored in API telemetry. Sheets read and write channels are independently paced at 1.3 seconds/request (about 46/minute each); GSC requests share 0.15-second spacing (about 400/minute), including inspection worker threads. Other applications can still consume shared quota. GA timezone lookup now uses one day instead of the full history since launch.

Reference limits checked September 10: standard GA core 200,000 tokens/property/day, 40,000/hour, 14,000/project/property/hour, 10 concurrent/property. These are Google quota units, not OpenAI tokens; actual consumed/remaining responses are recorded. Search Analytics site and user request limits are 1,200/minute, with additional unexposed short/long load quotas. URL Inspection is 2,000/site/day and 600/site/minute; this monitor schedules 30/day by default (hard limit 100). Sheets permits 60 reads and 60 writes/minute/user/project, and 300 each/minute/project. A successful small-site measurement cannot guarantee future GSC load acceptance; review HTTP errors and quota snapshots as data grows. API Usage follows the existing 90-day verified-archive retention policy.

Capacity estimate: querying 5,000 candidates is one request per language/date, not one request per keyword. Nine languages over seven finalized dates require at most 63 current-pool requests plus 63 baseline-pool requests when all baselines are eligible, before retries. A weekly or monthly collection adds at most 18 query-pool requests for its current and baseline periods. The actual finalized window can be shorter. Page collection is a separate cost: one ranking request per language, then one exact-page request per selected page (plus pagination if needed); manually maintained URLs can increase this count. These are application request estimates, not GSC load-token estimates or a bound on the entire scheduled job.

Sources: [GA Data API quotas](https://developers.google.com/analytics/devguides/reporting/data/v1/quotas), [Search Console limits](https://developers.google.com/webmaster-tools/limits), [Sheets limits](https://developers.google.com/workspace/sheets/api/limits), [model pricing](https://developers.openai.com/api/docs/models/gpt-5.6-sol).
