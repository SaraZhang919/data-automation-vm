"""
weekly_page_gsc.py — Pull GSC weekly data by page per subdomain.
Pages: Top 30 by clicks + all URLs in Page name sheet.
...
def get_week_range(ref_tuesday):
    """Get the Sunday-Saturday week that ended before this Tuesday."""
    # Tuesday run covers last Sunday-Saturday
    dow = ref_tuesday.weekday()  # Tue=1
    days_since_sunday = (dow + 1) % 7
    end = ref_tuesday - timedelta(days=days_since_sunday + 1)  # last Saturday
...
def get_top_pages_gsc(gsc, gsc_property, start, end, limit=TOP_PAGES_COUNT):
    """Get top pages by clicks from GSC."""
    body = {
        "startDate": start.strftime("%Y-%m-%d"),
        "endDate": end.strftime("%Y-%m-%d"),
        "dimensions": ["page"],
...
def get_prev_from_sheet(rows, url):
    """Get previous metrics for a URL from sheet."""
    for row in rows[1:]:
...
def main():
    ...
    print(f"Running Weekly by Page GSC for {date_label}")

    gsc = get_gsc_client()
    sheets = get_sheets_client()
    tab = SHEET_NAMES["weekly_page_gsc"]
    ...
    for prop in PROPERTIES:
        ...
        for url in all_urls:
            ...
            # High impression + low CTR combo alert
            combo_alert = ""
    ...
    print(f"Weekly by Page GSC complete. {len(all_new_rows)} rows written, {len(highlight_ops)} highlights applied.")
