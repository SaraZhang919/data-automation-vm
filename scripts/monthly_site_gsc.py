"""
monthly_site_gsc.py — Pull GSC monthly site-level data per subdomain.
...
def main():
    ...
    print(f"Running Monthly Site GSC for {date_label}")

    gsc    = get_gsc_client()
    sheets = get_sheets_client()
    tab    = SHEET_NAMES["monthly_site_gsc"]
    ...
    for prop in PROPERTIES:
        ...
        lvl = thresholds.monthly_site_gsc_clicks(cur["clicks"], prev["clicks"])
        if lvl: highlight_ops.append((row_index, COL_CLICKS_CHANGE, lvl))

        lvl = thresholds.monthly_site_gsc_impressions(cur["impressions"], prev["impressions"])
    ...
    print(f"Monthly Site GSC complete. {len(all_new_rows)} rows written, {len(highlight_ops)} highlights applied.")
